...
<body>
	...
	<%-- 
	 ...
	 <p>
	 	contains : <!-- 문자열 A에 문자열B가 포함되어 있는지 확인한다 -->
	 	<c:if test="${fn:contains(fileName , '.png') }">
	 		png확장자 입니다.
	 	</c:if>
	 </p>
	 <p>
	 	substring : <!-- A에서 index 번호 B부터 C까지 해당하는 문자열을 반환 -->
	 	<c:set var="fileExtention" value="${fn:substring(fileName,10,13) }" />
	 	${fileExtention }
	 </p>
